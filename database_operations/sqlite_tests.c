 ...
 *
 * The field tests go from smallest to largest. For example, the first test checks
 * if the value is a `NULL`. If not that, the next check is for
 * `INTEGER`, and so on. If all tests fail, the value is assumed to be
 * `TEXT`.
 *
 * The field tests are cumulative such that a field can never return
 * to a smaller data type. For example, once a field is determined to be
 * `INTEGER`, it can never go back to `NULL` even if all subsequent values
 * are indeed `NULL`.
 *
 ...
 * @param user_data Pointer to a Column_Definition structure.
 *  */
